...
class Filter_Migration {
    ...
    public function up()
    {
        $this->database = new Asatru\Database\Migration('Filter', $this->connection);
        $this->database->drop();
        $this->database->add('id INT NOT NULL AUTO_INCREMENT PRIMARY KEY');
        $this->database->add('expression VARCHAR(512) NOT NULL');
        $this->database->add('type VARCHAR(512) NOT NULL');
